      ...
      <Card>
        ...
        <CardContent className="pt-0">
              ...
              <TabsTrigger value="active" className="gap-2">
                ...
                <Badge variant="secondary" className="ml-1">
                  {filteredOrders.filter(o => {
                    ...
                    return !o.delivery_complete && !allShipped && (o.fulfillment_percentage || 0) < 100;
                  }).length}
                </Badge>
              </TabsTrigger>
              <TabsTrigger value="ready_to_ship" className="gap-2">
          ...
          {(() => {
            ...
            const readyToShipOrders = sortOrders(filteredOrders.filter(o => !isOrderFinished(o) && (o.fulfillment_percentage || 0) >= 100));
                      ...
                      <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-3 mb-2">
                            ...
                            <Badge variant="outline" className="gap-1 text-xs border-primary/50 text-primary shrink-0">
                          ...
                          {/* Show preparing batches info */}
                          {preparingBatches > 0 && (
                            <div className="flex flex-wrap items-center gap-1.5 mt-2">
                              <BoxIcon className="h-3.5 w-3.5 text-warning shrink-0" />
                              <span className="text-xs text-warning font-medium mr-1">Preparing:</span>
                              <Badge className="text-xs py-0 px-1.5 bg-warning/20 text-warning border-warning/50">
                                {preparingBatches} batch{preparingBatches !== 1 ? 'es' : ''} preparing
                              </Badge>
                            </div>
                          )}
                        </div>
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <div className="flex items-center gap-2 text-sm whitespace-nowrap">
                                ...
                                <span className={daysUntil <= 3 ? "text-destructive font-medium" : daysUntil <= 7 ? "text-warning font-medium" : "text-muted-foreground"}>
                                  {daysUntil < 0 ? `${Math.abs(daysUntil)} days overdue` : `${daysUntil} days left`}
                                </span>
                              </div>
                            </TooltipTrigger>
                            <TooltipContent>
                              <p>Delivery date: {format(new Date(order.delivery_date), 'MMM d, yyyy')}</p>
                            </TooltipContent>
                          </Tooltip>
                          <div className="flex items-center gap-1">
                            {ordersWithConstructions.has(order.id) && (
                        ...
                        <div key={order.id} id={`order-${order.id}`} className={`relative block p-4 rounded-lg border bg-card transition-colors ${(isAdmin || isManager) ? 'hover:bg-muted/50 cursor-pointer' : ''}`} onClick={() => (isAdmin || isManager) && navigate(`/orders/${order.id}`)}>
                          ...
                          <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                            <div className="flex-1 min-w-0">
                              ...
                              {/* Show shipped batches info */}
                              {orderBatches.length > 0 && (
                                <div className="flex flex-wrap items-center gap-1.5 mt-2">
                                  <Truck className="h-3.5 w-3.5 text-success shrink-0" />
                                  <span className="text-xs text-success font-medium mr-1">Shipped:</span>
                                  <Badge className="text-xs py-0 px-1.5 bg-success/20 text-success border-success/50">
                                ...
                                <TooltipTrigger asChild>
                                  <div className="flex items-center gap-2 text-sm whitespace-nowrap">
                                    <span className="text-muted-foreground">
                                      Delivered {format(new Date(order.delivery_date), 'MMM d, yyyy')}
                                    </span>
                                  </div>
                                </TooltipTrigger>
                                <TooltipContent>
